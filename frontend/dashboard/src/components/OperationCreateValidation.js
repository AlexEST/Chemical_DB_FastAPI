...
const ValidateOperationCreation = async (values) => {
    ...
    if (!values.substance) {
        ...
        if (locale === 'en') {
            errors.substance = 'The substance is required';
        } else if (locale === 'ru') {
            errors.substance = 'Выберите название вещества';
        } else {
            errors.substance = 'Valige aine nimi';
        }
    ...
    if (!values.type) {
        const locale = String(i18nProvider.getLocale())
        if (locale === 'en') {
            errors.type = 'Type operation is required';
        } else if (locale === 'ru') {
            errors.type = 'Выберите тип операции';
        } else {
            errors.type = 'Valige operatsiooni tüüp';
        }
    }
    if (!values.amount) {
        const locale = String(i18nProvider.getLocale())
    ...
    if (values.amount && values.substance && values.type === 'Remove') {
        const { data: db } = await dataProvider.getList('stock/substance', {
            pagination: { page: 1, perPage: 1 },
            sort: { field: 'id', order: 'ASC' },
            filter: { name: values.substance },
        });
        if (!db) {
            const locale = String(i18nProvider.getLocale())
            if (locale === 'en') {
                errors.amount = 'Not available';
            } else if (locale === 'ru') {
                errors.amount = 'Нет данных';
            } else {
                errors.amount = 'Ei ole andmeid';
            }
        } else if ((db.amount - Math.abs(values.amount)) < 0) {
            const locale = String(i18nProvider.getLocale())
            if (locale === 'en') {
...
export default ValidateOperationCreation;